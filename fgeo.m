...
for i=1:cmplx(3).num(3).val
    ...
    k1 = cmplx(3).bndop(2).indx(i,1); s1 = cmplx(3).bndop(2).sgn(i,1);
    k2 = cmplx(3).bndop(2).indx(i,2); s2 = cmplx(3).bndop(2).sgn(i,2);
    if s1==1; if cmplx(2).bndop(1).sgn(k1,2)==1; id=2; else; id=1; end
    else; if cmplx(2).bndop(1).sgn(k1,2)==1; id=1; else; id=2; end; end
    if ~any(cmplx(2).bndop(1).indx(k1,id)==...
            cmplx(2).bndop(1).indx(k2,:))
        for j=3:cmplx(3).num(2).val(i)
            k2 = cmplx(3).bndop(2).indx(i,j); s2 = cmplx(3).bndop(2).sgn(i,j);
            if any(cmplx(2).bndop(1).indx(k1,id)==...
                    cmplx(2).bndop(1).indx(k2,:)); break; end
        end; end
    if s2==1; if cmplx(2).bndop(1).sgn(k2,2)==1; id2=1; else; id2=2; end
    else; if cmplx(2).bndop(1).sgn(k2,2)==1; id2=2; else; id2=1; end; end
    if cmplx(2).bndop(1).indx(k1,id)~=cmplx(2).bndop(1).indx(k2,id2)
        pause
    end
    
    cmplx(3).dir(i,:) = cross(s1*cmplx(2).dir(k1,:),s2*cmplx(2).dir(k2,:));
    cmplx(3).dir(i,:) = cmplx(3).dir(i,:)./norm(cmplx(3).dir(i,:));
...
% - fvol
% -------------------------------------------------------------------------
cmplx(1).fvol_orig  = zeros(cmplx(1).num(1).val,1);
...
for i=1:cmplx(1).num(1).val
    ...
    % compute fvol
    omega = 4*pi()/omega;  %omega=1;
    cmplx(1).omega(i) = omega;
    
    cmplx(1).fvol_orig(i)  = cmplx(1).fvol(i)  / ...
...
cmplx(2).fvol_orig  = zeros(cmplx(2).num(2).val,1);
for i=1:cmplx(2).num(2).val
    
    for j=1:cmplx(2).num(1).val(i)
        k = cmplx(2).bndop(1).indx(i,j);
        
        cmplx(2).fvol_orig(i) = cmplx(2).fvol_orig(i) + cmplx(1).fvol_orig(k);
        cmplx(2).fvol(i) = cmplx(2).fvol(i) + cmplx(1).fvol(k);
    end
    cmplx(2).fvol_orig(i)  = cmplx(2).fvol_orig(i)  / (2^(1)*cmplx(2).bfvol(i)^2);
...
% - fvol
% -------------------------------------------------------------------------
cmplx(3).fvol_orig  = zeros(cmplx(3).num(3).val,1);
cmplx(3).fvol  = zeros(cmplx(3).num(3).val,1);
for i=1:cmplx(3).num(3).val
    for j=1:cmplx(3).num(1).val(i)
        k = cmplx(3).bndop(1).indx(i,j);
        
        cmplx(3).fvol_orig(i) = cmplx(3).fvol_orig(i) + cmplx(1).fvol_orig(k);
        cmplx(3).fvol(i) = cmplx(3).fvol(i) + cmplx(1).fvol(k);
    end
    cmplx(3).fvol_orig(i)  = cmplx(3).fvol_orig(i)  / (2^(2)*cmplx(3).bfvol(i)^2);
...
% - fvol
% -------------------------------------------------------------------------
cmplx(4).fvol_orig  = zeros(cmplx(4).num(4).val,1);
cmplx(4).fvol  = zeros(cmplx(4).num(4).val,1);
for i=1:cmplx(4).num(4).val
    for j=1:cmplx(4).num(1).val(i)
        k = cmplx(4).bndop(1).indx(i,j);
        
        cmplx(4).fvol_orig(i) = cmplx(4).fvol_orig(i) + cmplx(1).fvol_orig(k);
        cmplx(4).fvol(i) = cmplx(4).fvol(i) + cmplx(1).fvol(k);
    end
    cmplx(4).fvol_orig(i)  = cmplx(4).fvol_orig(i)  / (2^(3)*cmplx(4).bfvol(i)^2);
